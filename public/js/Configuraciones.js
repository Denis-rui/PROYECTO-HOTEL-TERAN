window.inicializarConfiguraciones = () => {
  ...
  formulario.addEventListener("submit", function (e) {
    ...
    const datos = {
      ...
      porcentaje_penalidad: document.getElementById("porcentaje_penalidad")
        .value,
    };

    // Confirmación usando SweetAlert2
    Confirmar("¿Estás seguro de conservar los cambios realizados?").then(
      (confirmado) => {
  ...
  document
    .getElementById("btnNuevoTipoHabitacion")
  ...
  formTipo.addEventListener("submit", async (e) => {
    ...
    const id = document.getElementById("id-tipo").value.trim();
    ...
    try {
      const res = await fetch(BASE_URL + "Configuracion/guardarTipo", {
        method: "POST",
        headers: {
      ...
      if (!res.ok || !result) {
        mensajeModal.className = "div-mensaje-exito-error error";
        mensajeModal.textContent = "Error al guardar";
        return;
      }

      if (!result.exito) {
        mensajeModal.className = "div-mensaje-exito-error error";
        mensajeModal.textContent = result.mensaje || "Error al guardar";
        return;
      }
