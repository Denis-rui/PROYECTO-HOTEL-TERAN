...
class ReservaFormatterHelper
{
    ...
    public static function formatear($reserva): array
    {
        ...
        $habitacionesRelacionadas = is_iterable($reservaHabitacion)
            ? $reservaHabitacion
            : [$reservaHabitacion];
        ...
        if (
            ...
            if ($checkoutProgramadoFecha < $ahora) {
                $minutosCheckoutVencido = (int) floor(
                    ($ahora->getTimestamp() - $checkoutProgramadoFecha->getTimestamp()) / 60
                );
            } elseif (
        ...
        return [
            ...
            'habitacion' => $habitacionPrincipal
                ? 'Hab. ' . $habitacionPrincipal['numero_habitacion'] . ' - Piso ' . $habitacionPrincipal['piso']
                : '',
            ...
            'porcentaje_pago' => $total + $cargoTarde > 0
                ? round(($totalPagado / ($total + $cargoTarde)) * 100, 0)
                : 0,
    ...
    private static function obtenerNombreTipoDocumento($cliente): ?string
    {
        try {
            ...
            return DB::table('tipo_documento')
                ->where('id', $cliente->id_tipo_documento)
                ->value('nombre');
        } catch (\Throwable $e) {
            return null;
        }
